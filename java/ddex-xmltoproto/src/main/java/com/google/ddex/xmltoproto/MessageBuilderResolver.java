...
/**
 * The MessageBuilderResolver is responsible for returning an appropriate message builder for given
 * DDEX message input.
 */
public class MessageBuilderResolver {
  ...
  /** Hidden constructor. */
  private MessageBuilderResolver() {
  }

  /**
   * Returns the appropriate message builder for the supplied document. This method loads the
   * generated protocol buffer classes using reflection. For example, an input XML message with the
   * version "ern411" would resolve to the generated Protocol Buffer builder compatible with all of
   * ern's major version 4
   *
   * @param document The Document representing a DDEX XML message
   * @return The corresponding message builder
   * @throws MessageParseException If there was a problem loading a builder for the DDEX message,
   *     either there is no builder that is compatible with the supplied DDEX message, or an issue
   *     occurred when trying to load a class with reflection.
   */
  public static Message.Builder getBuilder(Document document) throws MessageParseException {
    ...
    if (namespace.equals("ern") || namespace.equals("ernm")) {
      ...
      throw new MessageParseException(
          "Unsupported message namespace: " + namespace + ", version: " + versionNumber);
    }
  ...
  /**
   * Returns the appropriate message builder for the supplied reader.
   *
   * @see MessageBuilderResolver#getBuilder(Document)
   * @param reader The reader for the DDEX XML message.
   * @return The corresponding message builder
   * @throws MessageParseException If there was a problem loading a builder for the DDEX message,
   *     either there is no builder that is compatible with the supplied DDEX message, or an issue
   *     occurred when trying to load a class with reflection.
   * @throws IOException If any IO error occurs handling the File
   */
  public static Message.Builder getBuilder(Reader reader)
      throws MessageParseException, IOException {
    Document document = getDocument(reader);
    return getBuilder(document);
  }

  private static Message.Builder dynamicBuilderLoader(String className, String methodName)
  ...
  private static Document getDocument(Reader reader) throws IOException {
    try {
  ...
  private static int getMeadMajorVersionNumber(Node root) throws MessageParseException {
    String schemaLocation = getSchemaLocationString(root);
    try {
  ...
  private static String getSchemaLocationString(Node root) throws MessageParseException {
    if (root.getAttributes().getNamedItem("xmlns:ern") != null) {
      return root.getAttributes().getNamedItem("xmlns:ern").getNodeValue();
    } else if (root.getAttributes().getNamedItem("xmlns:ernm") != null) {
      return root.getAttributes().getNamedItem("xmlns:ernm").getNodeValue();
    } else if (root.getAttributes().getNamedItem("xmlns:mead") != null) {
      return root.getAttributes().getNamedItem("xmlns:mead").getNodeValue();
    } else {
      throw new MessageParseException("No version defining attribute found.");
    }
  }
