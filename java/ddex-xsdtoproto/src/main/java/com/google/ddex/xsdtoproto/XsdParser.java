...
public class XsdParser {
  ...
  private final XsdParserReporter reporter;
  private XsdNamespaceMap namespaceMap;

  private XsdParser(Reader reader, XsdParserReporter reporter) {
  ...
  /**
   * Parse DDEX XSD, a schema representing either the MEAD or ERN standard.
   *
  ...
  public static ProtoSchema parse(Reader reader) throws XsdParseException {
    XsdParserReporter defaultReporter = new XsdParserReporter.DefaultXsdParserReporter();
    return new XsdParser(reader, defaultReporter).parseSchema();
  }
   ...
   *
   * @param reader The reader for the input DDEX XSD.
   * @param reporter Reference to a XsdParseReporter, which will store namespaces processed and
   *     warnings generated during parse.
   * @return The {@link com.google.ddex.xsdtoproto.ProtoSchema} representation of the XSD.
   * @throws XsdParseException If any problem occurred parsing the DDEX XSD
   */
  public static ProtoSchema parse(Reader reader, XsdParserReporter reporter)
      throws XsdParseException {
    return new XsdParser(reader, reporter).parseSchema();
  }

  private ProtoSchema parseSchema() throws XsdParseException {
    ...
    protoSchemaEntryMap.setRootNamespacePrefix(
        namespaceMap.getPrefix(inputSchema.getTargetNamespace()));
  ...
  private QName getAttributeTypeName(XmlSchemaAttribute attribute) {
    QName name = attribute.getSchemaTypeName();
