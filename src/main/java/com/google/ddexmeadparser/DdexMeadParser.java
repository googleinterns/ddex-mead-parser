...
import com.google.ddexmeadparser.experimental.SchemaSetMerger;
...
public class DdexMeadParser {
  ...
  public void exec()
    ...
    if (runtimeOptions.inputType == DdexMeadParserOptions.inputTypeValue.MESSAGE) {
      ...
      parseSchema();
    } else if (runtimeOptions.inputType == DdexMeadParserOptions.inputTypeValue.SCHEMA_SET) {
      parseSchemaSet();
    }
  }
  ...
  public void parseMead() throws MeadConversionException, InvalidOptionsException {
    logger.atInfo().log(
        "Started mead message CONVERSION on file: " + runtimeOptions.inputFile.getName());
  ...
  private void writeSchema(ProtoSchema schema) throws SchemaConversionException {
    ...
    for (String namespace : namespaces) {
      File file =
          new File(
              "./src/main/proto/" + rootNamespace + "/" + packageName + "/" + namespace + ".proto");
  ...
  private void parseSchemaSet() throws InvalidOptionsException, SchemaConversionException {
    assert (runtimeOptions.inputFile.isDirectory());
    ...
    for (File schemaFile : runtimeOptions.inputFileList) {
      ...
      StreamSource xsdFile = getStreamSource(schemaFile);

      SchemaConverter schemaConverter = new SchemaConverter();
      SchemaEntryMap schemaEntryMap = schemaConverter.convert(xsdFile);
      schemaSetMerger.addSchema(schemaEntryMap);
    }

    SchemaEntryMap finalMap = schemaSetMerger.merge();
    ProtoSchema protoSchema = new ProtoSchema(finalMap);
  ...
  private static DdexMeadParserOptions buildRuntimeOptions(CommandLine commandLineInput)
    ...
    if (commandLineInput.hasOption("inputType")
        && commandLineInput.getOptionValue("inputType").equals("message")) {
      runtimeOptions.inputType = DdexMeadParserOptions.inputTypeValue.MESSAGE;
    } else if (commandLineInput.hasOption("inputType")
        && commandLineInput.getOptionValue("inputType").equals("schema")) {
      runtimeOptions.inputType = DdexMeadParserOptions.inputTypeValue.SCHEMA;
    } else if (commandLineInput.hasOption("inputType")
        && commandLineInput.getOptionValue("inputType").equals("schema_set")) {
      runtimeOptions.inputType = DdexMeadParserOptions.inputTypeValue.SCHEMA_SET;
    } else {
    ...
    if (args.length == 1) {
      ...
      for (int i = 0; i < args.length; i++) {
        File meadXsd = new File(args[i]);
        if (meadXsd.exists()) {
          runtimeOptions.inputFileList.add(meadXsd);
        } else {
          throw new InvalidOptionsException("XML file input does not exist.");
        }
      }
